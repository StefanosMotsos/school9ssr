...
//    }
    ...
    public TeacherReadOnlyDTO saveTeacher(TeacherInsertDTO dto)
        ...
        try {
            if (dto.vat() != null &&
                    teacherRepository.findByVat(dto.vat()).isPresent()) {
            ...
            Region region = regionRepository.findById(dto.regionId())
                    .orElseThrow(() ->
                            new EntityInvalidArgumentException("Region id=" + dto.regionId() + " invalid"));
            ...
            log.error("Save failed for teacher with vat={}: Teacher already exists",
                    dto.vat());     //Structured Logging
            throw  e;
        } catch (EntityInvalidArgumentException e) {
            log.error("Save failed for teacher with vat={}: Region id={} invalid",
                    dto.vat(), dto.regionId());
        }
